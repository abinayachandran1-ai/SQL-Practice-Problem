...
SELECT CustomerID, CONCAT(FirstName, ' ', IFNULL(LastName, '')) AS CustomerName, City 
FROM Customers;
...
SELECT *
FROM Customers
WHERE FirstName LIKE "%Kumar" OR LastName LIKE '%Kumar';
...
SELECT CONCAT(FirstName, ' ', IFNULL(LastName, '')) AS Customer
FROM Customers;
...
SELECT ItemName, Price,
	ROUND(Price * 1.05, 2) AS Service_Charge
FROM MenuItems;
...
SELECT ROUND(Avg(TotalAmount),2) AS Average_Order_Amount
FROM Orders;
...
SELECT ROUND(Avg(FoodRating), 2) AS Customer_Review_Rating
FROM Reviews;

# 50.	Display the average review rating rounded to two decimal places. 
SELECT ROUND(Avg(DeliveryRating), 2) AS Review_Rating
FROM Reviews;
...
SELECT PaymentMethod, COUNT(TransactionID) AS Number_of_Transactions
FROM Payments
GROUP BY PaymentMethod;

# 55.	Display the number of reviews for each rating. 
SELECT DeliveryRating, COUNT(ReviewComment) AS Reviews_Rating
FROM Reviews
...
SELECT 
    ...
    mc.CategoryName
FROM MenuItems AS m
INNER JOIN MenuCategories AS mc
...
SELECT
    rs.RestaurantName,
    Avg(r.FoodRating) AS average_rating
FROM Restaurants rs
INNER JOIN Reviews r on rs.Restaurantid = r.Restaurantid
...
SELECT
    r.Restaurantname,
    r.City,
    Avg(m.Price) AS Average_Menu_Price
FROM Restaurants r
INNER JOIN MenuItems m 
	ON r.RestaurantID = m.RestaurantID
...
SELECT
	OrderID,
    MONTHNAME(OrderDate) AS Month_Order
FROM Orders;

# 100.	Display the day of the month for every order. 
SELECT
	OrderID,
    DayofMonth(OrderDate) AS Day_Of_Month
FROM Orders;
...
WHERE OrderDate
BETWEEN 
	DATE_SUB((SELECT MAX(OrderDate) FROM orders), INTERVAL 29 DAY)
AND 
...
SELECT
	MONTH(PaymentDate),
	SUM(Amount) AS Revenue_Of_Completed_Payments
FROM Payments
WHERE PaymentStatus = "Success"
...
SELECT 
    MONTH(OrderDate) AS OrderDay, 
    COUNT(*) AS MonthOrder_Count
FROM Orders
...
SELECT ROUND(Avg(TimeDiff(DeliveryTime, PickupTime))) AS Avg_Delivery_Time
FROM Delivery;
...
SELECT
    DENSE_RANK() OVER (ORDER BY SUM(o.TotalAmount) DESC) AS Rank_No,
    c.CustomerID,
    c.FirstName,
    SUM(o.TotalAmount) Lifetime_Spending
FROM Customers c
INNER JOIN Orders o
	ON c.CustomerID = o.CustomerID
GROUP BY c.CustomerID, c.FirstName;
...
SELECT
    ...
    (
        ...
        FROM Payments p2
        WHERE p2.PaymentID 
        BETWEEN (p1.PaymentID - 2) AND (p1.PaymentID)
    ) AS Moving_Average
FROM Payments p1
...
SELECT
	c1.CustomerID,
    c1.FirstName,
	SUM(o1.TotalAmount) as Customer_spend,
    ( SELECT 
		Avg(o2.TotalAmount)
        FROM Orders AS o2
...
HAVING customer_spend > Avg_Spend_Amt ;
...
FROM Orders O
JOIN (
    SELECT RestaurantID, AVG(TotalAmount) AS AvgOrderVal
    FROM Orders
    GROUP BY RestaurantID
) AvgTable ON O.RestaurantID = AvgTable.RestaurantID
WHERE O.TotalAmount > AvgTable.AvgOrderVal;
...
SELECT 
    ...
    TotalAmount,
    CASE 
        WHEN TotalAmount < 300 THEN 'Low Value'
        WHEN TotalAmount BETWEEN 300 AND 800 THEN 'Medium Value'
        ELSE 'High Value'
...
SELECT C.CustomerID, C.FirstName, C.LastName, SUM(O.TotalAmount) AS LifetimeSpending
FROM Customers C
...
SELECT R.RestaurantName, AVG(Rev.FoodRating) AS AvgRating
FROM Restaurants R
JOIN Reviews Rev ON R.RestaurantID = Rev.RestaurantID
GROUP BY R.RestaurantID, R.RestaurantName
HAVING AVG(Rev.FoodRating) > 4.5;

# 140.	Calculate the Customer Lifetime Value (CLV) for every customer. 
SELECT C.CustomerID, C.FirstName, C.LastName, IFNULL(SUM(O.TotalAmount), 0) AS CLV
FROM Customers C
...
SELECT YEAR(PaymentDate) AS PayYear, MONTH(PaymentDate) AS PayMonth, SUM(Amount) AS MonthlyRevenue
FROM Payments
WHERE PaymentStatus = 'Success'
GROUP BY YEAR(PaymentDate), MONTH(PaymentDate);

# 142.	Display the total number of orders placed during each hour of the day. 
SELECT HOUR(OrderDate) AS OrderHour, COUNT(*) AS TotalOrders
FROM Orders
GROUP BY HOUR(OrderDate)
ORDER BY OrderHour ASC;

# 143.	Display the average delivery time for each delivery partner. 
SELECT DP.PartnerID, DP.PartnerName, AVG(TIMESTAMPDIFF(MINUTE, D.PickupTime, D.DeliveryTime)) AS AvgDeliveryMinutes
FROM DeliveryPartners DP
...
WHERE D.DeliveryStatus = 'Delivered'
GROUP BY DP.PartnerID, DP.PartnerName
ORDER BY AvgDeliveryMinutes ASC
LIMIT 1;

# 145.	Rank payment methods based on completed transactions. 
SELECT PaymentMethod, COUNT(*) AS CompletedCount
FROM Payments
WHERE PaymentStatus = 'Success'
GROUP BY PaymentMethod
...
SELECT 
    R.RestaurantName,
    SUM(O.TotalAmount) AS RestaurantRevenue,
    ROUND((SUM(O.TotalAmount) / TotalTable.GrandTotal) * 100, 2) AS PercentageContribution
FROM Restaurants R
JOIN Orders O ON R.RestaurantID = O.RestaurantID
JOIN (
...
WHERE O.OrderStatus = 'Delivered'
GROUP BY R.RestaurantID, R.RestaurantName, TotalTable.GrandTotal;
...
SELECT C.CustomerID, C.FirstName, C.LastName, COUNT(DISTINCT O.RestaurantID) AS UniqueRestaurants
FROM Customers C
JOIN Orders O ON C.CustomerID = O.CustomerID
GROUP BY C.CustomerID, C.FirstName, C.LastName
...
SELECT 
    Cust.TotalCustomers,
    Rest.TotalRestaurants,
    Ord.TotalOrders,
    Pay.CompletedPayments
FROM 
    (SELECT COUNT(*) AS TotalCustomers FROM Customers) Cust,
    (SELECT COUNT(*) AS TotalRestaurants FROM Restaurants) Rest,
    (SELECT COUNT(*) AS TotalOrders FROM Orders) Ord,
    (SELECT COUNT(*) AS CompletedPayments FROM Payments WHERE PaymentStatus = 'Success') Pay;
...
SELECT 
    ...
    ROUND(AVG(Rev.FoodRating), 2) AS AvgCustomerRating,
    ROUND(AVG(TIMESTAMPDIFF(MINUTE, D.PickupTime, D.DeliveryTime)), 2) AS AvgDeliveryMinutes
FROM Restaurants R
LEFT JOIN Orders O ON R.RestaurantID = O.RestaurantID AND O.OrderStatus = 'Delivered'
